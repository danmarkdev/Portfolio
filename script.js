...
.tech-icon-box.tip-open{
  transform:translateY(-5px);
  box-shadow:0 14px 28px -8px rgba(18,22,43,.22);
  z-index:5;
}
.tech-icon-box.tip-open > .fb-icon{box-shadow:var(--shadow-card-hover)}
...
.tech-tip-icon.fb-icon{
  background: var(--accent-soft) !important;
  border: 1px solid var(--border);
}

/* rich tooltip card — icon + name + category tag + description,
   shown above the icon on hover */
.tech-tip{
  position:absolute;
...
.tech-icon-box.tip-open .tech-tip{
  opacity:1;
  visibility:visible;
  transform:translateX(calc(-50% + var(--tip-shift, 0px))) translateY(0);
}
...
.tech-tip-icon img,
.tech-tip-icon svg{width:18px;height:18px}
.tech-tip-titles{display:flex;flex-direction:column;gap:.2rem;min-width:0}
.tech-tip-name{
  font-family: var(--font-heading);
  font-size:.95rem;
  font-weight:700;
...
/* little pointer under the card, aimed at the icon */
.tech-tip::after{
  content:'';
  position:absolute;
  top:100%;
...
/* Flip tooltip below the icon when there isn't room above it */
...
.tech-icon-box.tip-flip.tip-open .tech-tip{
  transform:translateX(calc(-50% + var(--tip-shift, 0px))) translateY(0);
}
...
/* SKILLS — DESKTOP: give the grid a little more breathing room */
@media(min-width:961px){
  .tech-icon-grid{gap:1.15rem}
}

/* SKILLS — MOBILE/TABLET: smaller tiles, tooltip card recentres and
   shrinks so it never overflows the viewport */
@media(max-width:700px){
  .tech-panel{padding:2rem 1.25rem}
  .tech-icon-box{width:58px;height:58px;border-radius:14px}
  .tech-icon-box > .fb-icon img,
  .tech-icon-box > .fb-icon svg{width:28px;height:28px}
  .tech-tip{width:220px;padding:.85rem .95rem}
  .tech-tip-name{font-size:.88rem}
  .tech-tip-desc{font-size:.76rem}
}
@media(max-width:480px){
  .tech-icon-box{width:50px;height:50px}
  .tech-icon-box > .fb-icon img,
  .tech-icon-box > .fb-icon svg{width:24px;height:24px}
  .tech-tip{width:190px}
}
